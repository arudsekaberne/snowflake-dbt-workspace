...
CREATE OR REPLACE ROW ACCESS POLICY DEV_DBTGOVERN.POLICIES.NEIGHBOURHOOD_ACCESS_POLICY
AS (neighbourhood VARCHAR)
RETURNS BOOLEAN ->
    CASE
        WHEN CURRENT_ROLE() IN ('ACCOUNTADMIN') THEN TRUE
        WHEN CURRENT_ROLE() IN ('SYSADMIN') AND LOWER(neighbourhood) IN ('westminster') THEN TRUE
        ELSE FALSE
    END
;
...
ALTER TABLE DEV_BRONZE_ADF.AIRBNB."AirBnBReviews_View" ADD ROW ACCESS POLICY DEV_DBTGOVERN.POLICIES.CURRENT_YEAR_ACCESS_POLICY
    ON("date");
...
SELECT MIN("date"), MAX("date") FROM DEV_BRONZE_ADF.AIRBNB."AirBnBReviews_View" LIMIT 5;