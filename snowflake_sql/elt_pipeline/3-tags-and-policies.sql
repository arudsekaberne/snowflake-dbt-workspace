...
CREATE OR REPLACE ROW ACCESS POLICY DEV_DBTGOVERN.POLICIES.PREVIOUS_YEAR_LESS_REVIEW_ACCESS_POLICY_LANDING AS (date VARCHAR, review_count VARCHAR)
RETURNS BOOLEAN ->
    CASE
        WHEN CURRENT_ROLE() IN ('ACCOUNTADMIN') THEN TRUE
        WHEN CURRENT_ROLE() IN ('SYSADMIN') AND YEAR(date:: DATE) = YEAR(CURRENT_DATE) - 1 AND review_count:: INT <= 10 THEN TRUE
        ELSE FALSE
    END
;
